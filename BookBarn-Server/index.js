  ...
  try {
    ...
    app.post("/users", async (req, res) => {
      try {
        ...
        const userData = {
          ...newUser,
          role: newUser.role || "user", // Default to 'user'
        ...
        res
          .status(500)
          .send({ message: "Failed to save user", error: error.message });
      }
    });
    ...
    app.patch("/users/:email", async (req, res) => {
      ...
      if (!role || (role !== "admin" && role !== "seller" && role !== "user")) {
    ...
    app.post("/initiate-payment", async (req, res) => {
      try {
        ...
        if (!apiResponse.GatewayPageURL) {
          ...
          return res
            .status(500)
            .send({ message: "Payment gateway URL not found" });
        }
    ...
    app.post("/payment-success", async (req, res) => {
      ...
      if (!tran_id || !email) {
        return res
          .status(400)
          .send({ message: "Missing tran_id or email in query params" });
      }
